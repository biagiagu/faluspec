...
> Versión 0.6 · borrador · 2026-08-26
>
...
>
> **Los nombres de campo y los valores del formato están en inglés; esta documentación, en español.**
> Lo escrito en un backlog queda en el repositorio de quien lo adopte y no se puede traducir después
> sin romperle el trabajo; un documento sí. La prosa de cada proyecto —las historias, los escenarios,
> los títulos— va en el idioma que quiera: el formato no la mira.
...
3. Si el ítem que lo contiene está en estado `done`, **declara** al menos un `anchor` y una `verify`.
4. Cada `anchor` resuelve a algo que existe, o es `none` con su motivo (§2.3).
5. Su `verify` declara un tipo de los definidos en §2.4.
6. Su `verify` **puede fallar si el criterio se incumple**.
...
`anchor: none — <motivo>` es una declaración válida y suficiente. Lo que la regla 3 prohíbe es el
...
| `none — <motivo>` | El criterio no tiene dónde anclarse, y eso es permanente. |
...
`none` **es una declaración, no un hueco.** Lleva motivo obligatorio, y existe para tres casos que
aparecen de verdad:
...
`none` y la verificación es `static`.
...
| `integration` | Test contra dependencias reales (base de datos, servicios levantados). |
| `e2e` | Recorrido completo por la interfaz o la API pública. |
| `static` | Chequeo mecánico sobre el repositorio, no sobre el sistema corriendo. |
...
`static` es para los criterios que afirman algo del **repositorio** y no del sistema: que un valor
...
siendo `static`, y llamarlo `manual` mentiría sobre quién decide el resultado.
...
**orden de trabajo**. Un criterio con `verify` completa le dice a quien lo implemente —persona o
...
| `epic` | sí | Debe existir. Debe coincidir con el prefijo del `id`. |
| `title` | sí | Una línea, en imperativo o sustantivo. |
| `priority` | sí | Uno de los niveles definidos por el proyecto en su constitución. |
| `status` | sí | Uno de §7. |
| `status_since` | condicional | Obligatorio cuando el estado es `done` o `blocked`. |
| `blocked_by` | condicional | Obligatorio cuando el estado es `blocked`. **Una línea corta**: qué se espera y de quién. El detalle va al historial — este campo termina dentro de una celda de tabla. |
| `depends_on` | no | Lista de identificadores de ítems que deben estar `done` antes de empezar éste. |
| `milestone` | no | El corte de release al que pertenece. Debe existir. Un ítem sin `milestone` es legítimo: todavía no entró en ningún corte. |
...
1. Todo `depends_on` apunta a un ítem que existe.
2. El grafo de dependencias no tiene ciclos.
3. Un ítem `done` tiene `status_since`.
4. Un ítem `done` tiene al menos un criterio, y todos sus criterios son válidos según §2.2.
5. Un ítem no puede estar `done` si alguno de sus `depends_on` no lo está.
6. Un ítem `blocked` tiene `blocked_by`, propio o heredado de su épica (§4).
7. Un ítem `done` declara su impacto (§3.5), aunque sea «ninguno».
8. Si declara `milestone`, ese hito existe.
...
`blocked_by` es una línea de texto libre, no un sistema de tickets.
...
Adoptar YAML entero traería dos cosas que no queremos. Su **tipado implícito**: un `title: no` se
...
Sí puede declarar un **`blocked_by` propio**, cuando todos o casi todos sus ítems esperan lo mismo,
...
**La relación entre ítem e hito vive en el ítem**, en su campo `milestone`. El hito declara únicamente lo
...
| `blocked` | No puede avanzar por una causa externa. Qué se espera va en `blocked_by`; el detalle, en el historial. |
...
**`status_since` es desde cuándo está así**, no cuándo alguien lo pasó a ese estado. Para un ítem
...
| Bloqueos | los `blocked_by` de ítems y épicas, agrupados por causa |
| Cobertura por criterio | campos `verify` cruzados con los tests que existen |
| Story map | el campo `milestone` de cada ítem, cruzado con las épicas |
...
| 0.2 | 2026-08-25 | Tipo de verificación `static` · ancla a archivo entero y ancla `none` declarada · símbolos no exportados y un nivel de propiedad · `blocked_by` · la verificación como encargo. |
| 0.3 | 2026-08-26 | Impacto declarado en `done` · la verificación tiene que poder fallar · `static` definida por lo que es y no por dónde corre · `blocked_by` de épica · se puede nacer en cualquier estado salvo `done` · el historial cubre decisiones, no sólo implementación · ancla a directorio. |
| 0.4 | 2026-08-26 | Campo `milestone` en el ítem: la relación con el corte de release vive en el ítem, y el hito deja de llevar su lista de contenido. |
...
| **0.6** | 2026-08-26 | **Cambio incompatible.** Los nombres de campo y los valores pasan a inglés, porque el formato es público (decisión 003). Ver la tabla de migración abajo. |
...
vistas** y encontrar que una de ellas era imposible de generar ([`VISTAS.md`](VISTAS.md) §7); la 0.6,
de decidir que esto lo puede usar cualquiera:
...
Renombrar en el encabezado de cada ítem y épica:
...
| `prioridad` | `priority` |
| `estado` | `status` |
| `fecha_estado` | `status_since` |
| `bloqueado_por` | `blocked_by` |
| `depende_de` | `depends_on` |
| `hito` | `milestone` |

Y en cada criterio: `ancla` → `anchor`, `verifica` → `verify`. Los valores que cambian son tres:
`integracion` → `integration`, `estatica` → `static`, `ninguna` → `none`. Los estados (`todo`, `wip`,
`done`, `blocked`) y los otros tipos de verificación ya estaban en inglés — la mezcla que había es
justamente lo que esta versión termina.

Las secciones del cuerpo que el formato lee también: `## Impacto` pasa a `## Impact`.

**Hasta la 1.0 el formato puede volver a romper compatibilidad.** Quien adopte hoy tiene que saber que
va a migrar; a cambio, hasta la 1.0 sus hallazgos todavía pueden cambiar el formato.
